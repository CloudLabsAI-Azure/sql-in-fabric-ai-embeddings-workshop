...
1. Back on the Microsoft Fabric Database Home Page, click the **New Workspace tile** to open the **Create a workspace blade** on the right side.
    ...
    !IMAGE[A picture of the Microsoft Fabric developer experience Home Page](media/image1.png){500}

1. In the **Create a workspace blade**, 

    !IMAGE[A picture of the Create a workspace blade on the right side of the page](media/image#.png){550}

    double check to make sure the cursor is in the **Name** field 
    
    !IMAGE[A picture of the cursor in the Name field](media/image#.png){550}
    
    and use the **following text for your Workspace Name**:

    +++FabricWorkspace@lab.LabInstance.Id+++
    
    !IMAGE[A picture of using the supplied text as the workspace name in the name field](media/image#.png){800}
...
1. Next, click the **green Apply button** on the **bottom left** of the Create a workspace blade.
    ...
    !IMAGE[A picture of clicking the **green Apply button** on the **bottom left** of the Create a workspace blade](media/image#.png){550} 

1. On the following page, you may get a popup titled "Introducing task flows (preview)". Click the green **Got it** button.

    !IMAGE[A picture of clicking the green Got it button to close a feature preview popup](media/image#.png){500}
...
1. On the Microsoft Fabric Workspace page, click the **New item** button on the top right of the page.
    ...
    !IMAGE[A picture of clicking the New item button on the top right of the page](media/image#.png){550}

1. In the **New item** blade on the right, 

    !IMAGE[A picture of the new item blade on the right of the page](media/image#.png){800}

    use the **Filter by item type search box** in the upper right
    
    !IMAGE[A picture of using the Filter by item type search box in the upper right of the new item blade](media/image#.png){300}
    
    to enter **SQL**

    !IMAGE[A picture of entering SQL into the item type search box](media/image#.png){800}

1. With the New item results filtered down, click on the **SQL database (preview)** tile.

    !IMAGE[A picture of clicking on the SQL database (preview) tile](media/image#.png){600}
...
1. In the **New SQL database** dialog window,
    ...
    !IMAGE[A picture of the New SQL database dialog window](media/image#.png){400}
    ...
    !IMAGE[A picture of entering the database name into the New SQL database dialog box](media/image#.png){400}

1. With the database name entered, click the **green Create button**.

    !IMAGE[A picture of clicking the green Create button in the New SQL database dialog box](media/image#.png){400}


1. Once the database is finished creating, 

    !IMAGE[A picture of the database creating](media/image#.png){400}
    ...
    !IMAGE[A picture of the SQL database details home page](media/image#.png)
...
1. We need some sample data in the database to work with. We can easily do this with the **Sample data** tile right on the database home page. Click the **Sample data** tile right on the database home page.
    ...
    !IMAGE[A picture of clicking the Sample data tile right on the database home page](media/image#.png){700}
...
1. In the upper right corner of the database home page, you will see a notification indicating that the data is being loaded into the database.
    ...
    !IMAGE[A picture of a notification indicating that the data is being loaded into the database](media/image#.png){400}
...
1. Allow this process to run (about 30-60 seconds) until you see a notification indicating that the data was successfully loaded into the database appearing again, in the upper right corner.
    ...
    *Go ahead, check email on your phone...it's ok....*
    ...
    !IMAGE[A picture of a notification indicating that the data was successfully loaded into the database](media/image#.png){400}
...
1. Also, once the data has finished loading, the middle of home page will change show a **Query, preview, or connect your data** message and image.
    ...
    !IMAGE[A picture of the middle of home page changing to show a Query, preview, or connect your data message and image](media/image#.png){700}
...
1. To start, look at the **Database Explorer** area on the left of the page. Here, click the dropdown arrow next to the database 
    ...
    !IMAGE[A picture of the Database Explorer area on the left of the SQL database details page](media/image#.png){400}

    to see a list of database schemas.

    !IMAGE[A picture of a list of database schemas in the newly created database](media/image#.png){300}

    and you can further expand the **SalesLT** schema to see object types

    !IMAGE[A picture of expanding the SalesLT schema to see object types](media/image#.png){300}

    as well as objects in the object type folders.

    !IMAGE[A picture of expanding the tables folder to see all the tables in the SalesLT database schema](media/image#.png){300}

1. If not already done, expand the **SalesLT** schema, followed by expanding the **Tables** folder. Then click on the **Address** table.

    !IMAGE[A picture of expanding the SalesLT schema, followed by expanding the Tables folder, then clicking on the Address table](media/image#.png){300}

1. You can see in the editor window, a read only **Data preview** of the contents of the Address table.

    !IMAGE[A picture of a read only data preview of the Address table](media/image#.png)
...
1. After browsing the data in the Address table, **close** the Data preview by clicking on the **X** next to the **Address Data preview tab**.
    ...
    !IMAGE[A picture of closing the Data preview by clicking on the X next to the Address Data preview tab](media/image#.png){600}

1. Now, click the **New Query** button on the tool bar
    
    !IMAGE[A picture of clicking the New Query button on the tool bar](media/image#.png){700}

    to open a new query editor window so we can work directly with the database. 

    !IMAGE[A picture of opening a new query editor window so one can work directly with the database](media/image#.png){800}

1. To remove the gray **Copilot Preview Banner**, click the X on the right side.

    !IMAGE[A picture of removing the gray Copilot Preview Banner by clicking the X on the right side](media/image#.png){700}
...
1. Once the banner is gone, be sure to click in the SQL editor sheet so that the next step will copy the code to the correct location.
    ...
    !IMAGE[A picture of clicking in the SQL editor sheet so that the next step will copy the code to the correct location](instructions281384/Screenshot 2025-01-10 at 10.00.59 AMw.png){600}
...
1. Once the code is in the query editor, **click the Run button**.
    ...
    !IMAGE[A picture of clicking the run button in the query editor](media/image#.png){500}

1. You will see the **results** of the query on the **bottom of the query editor**.

    !IMAGE[A picture of the results of the query on the bottom of the query editor](media/image#.png){800}

1. Starting on the left side of the **Results area**, there are 3 options for exporting the data.

    !IMAGE[A picture of the 3 options for exporting the data in the results area](media/image#.png){700}

1. From left to right, first is **download results as .xlsx (Excel) file**.

    !IMAGE[A picture of the download results as .xlsx (Excel) file option](media/image#.png){600}

1. Next, is **download as a .csv file**

    !IMAGE[A picture of the download as a .csv file option](media/image#.png){500}

1. With the last option being **download as a .json file**.

    !IMAGE[A picture of the download as a .json file option](media/image#.png){500}

1. And looking at the right side of the results area, you can use the **Copy** button to copy the results in multiple formats.

    !IMAGE[A picture of using the Copy button to copy the results in multiple formats](media/image#.png){700}

1. Now, **refresh the browser page** by clicking the refresh icon on the Edge Browser toolbar.

    !IMAGE[A picture of clicking the refresh icon on the Edge Browser toolbar](media/image#.png){600}

1. The query editor you were working with is no longer in the main window

    !IMAGE[A picture showing the query editor you were working with is no longer in the main window](media/image#.png){600}
    ...
    !IMAGE[A picture of clicking on the editor sheet name in Explorer on the left side of the page in the Queries folder](media/image#.png){600}

1. Also, by **clicking on the 3 dots** next to a query editor sheet name, you can duplicate it, rename it, or delete it.

    !IMAGE[A picture of clicking on the 3 dots next to a query editor sheet name](media/image#.png){400}
...
1. To get ready for the next section, **Delete** any SQL you have in **the query editor** so that it is blank. Do not delete the entire query sheet.
    ...
    !IMAGE[A picture of a blank query editor because all the code was deleted from it](media/image#.png){600}
...
1. To use **Copilot chat**, click the **Copilot button** on the Database details homepage toolbar.
    ...
    !IMAGE[A picture of clicking the Copilot button on the Database details homepage toolbar](media/image#.png){650}

1. On the right side of the page, you will see the **Copilot Chat pane**.

    !IMAGE[A picture of the Copilot Chat pane](instructions281384/Screenshot 2025-01-21 at 1.17.36 PM copy.png){300}

    click the **green Get started button** on the bottom of the chat pane to continue.

    !IMAGE[A picture of clicking the green Get started button on the bottom of the chat pane to continue](instructions281384/Screenshot 2025-01-21 at 1.17.36 PM.png){300}
...
1. Let's use one of the suggested questions that the chat pane has offered us. Click the **Get Insights: Retrieve the total number of tables in my database.** button. This will add the text "Retrieve the total number of tables in my database." in the chat box on the bottom of the Chat Pane.
    ...
    !IMAGE[A picture of clicking the Get Insights: Retrieve the total number of tables in my database suggested query](instructions281384/Screenshot 2025-01-21 at 1.31.37 PM.png){300}

1. Once the text is in the chat text box, click the **Arrow/Paper Airplane Icon** on the right of the chat text box.

    !IMAGE[A picture of clicking the Arrow/Paper Airplane Icon on the right of the chat text box](instructions281384/Screenshot 2025-01-21 at 1.38.21 PM.png){400}

    and Copilot will start answering the question

    !IMAGE[A picture of copilot working on the answer with an animated color bar](instructions281384/Screenshot 2025-01-21 at 1.40.11 PM.png){400}

1. Once the answer is returned by Copilot,

    !IMAGE[A picture of copilot returning the answer and SQL](instructions281384/Screenshot 2025-01-21 at 1.42.24 PM.png){400}

    you can either **click the copy code button** and paste it into the query editor

    !IMAGE[A picture of clicking the copy code button and pasting it into the query editor](instructions281384/Screenshot 2025-01-21 at 1.43.45 PM.png){400}

    or click the **insert code button** to have it instantly pasted into the current active query editor sheet.

    !IMAGE[A picture of clicking the insert code button having it instantly pasted into the current active query editor sheet](instructions281384/Screenshot 2025-01-21 at 1.45.33 PM.png){400}

1. Once the code is copied into the query editor, you **click the run button** it to see the results.

    !IMAGE[A picture of clicking the run button in the query editor](instructions281384/Screenshot 2025-01-21 at 1.47.02 PM.png){500}
...
1. Try the following question suggestions in the chat pane and see the results. Feel free to run the SQL it provides from the answers or just move on to the next question:
    ...
    > !IMAGE[A picture of Copilot for SQL database encountering unexpected results](media/image#.png){300} 
    ...
    !IMAGE[A picture of asking copilot what is the biggest table in my database](media/image#.png){400}

    ```Question
    Are there any poorly performing indexes?
    ```

    !IMAGE[A picture of asking copilot Are there any poorly performing indexes](media/image#.png){400}
    ...
    !IMAGE[A picture of asking copilot to Help me create a table to store AI chat history and code to insert some sample rows](media/image#.png){400}

    ```
    Help me alter the Address table to add a Subdivision column.
    ```

    !IMAGE[A picture of asking copilot to Help me alter the Address table to add a Subdivision column](media/image#.png){400}
    ...
    !IMAGE[A picture of asking copilot What is an append only ledger table](media/image#.png){400}

    ```Question
    Does SQL in fabric support CLR?
    ```

    !IMAGE[A picture of asking copilot Does SQL in fabric support CLR](media/image#.png){400}

1. When done with the questions, you can **close the Copilot for SQL chat window** using the **X on the top right**.

    !IMAGE[A picture of closing the Copilot for SQL chat window** using the X on the top right](media/image#.png){400}
...
1. In an empty query editor, copy and paste the following code.
    ...
    !IMAGE[A picture of pasting -- list the top 5 colors of products into the query editor](instructions281384/Screenshot 2025-01-21 at 1.52.07 PM.png){800}
...
1. Then **press enter/return at the end of the line of text** in the query editor. On the bottom of the query editor, you should see that **"Copilot is working on it..."***
    ...
    !IMAGE[A picture of pressing enter/return at the end of the line of text you just pasted in](instructions281384/Screenshot 2025-01-21 at 1.53.27 PM.png){300}

1. Back in the query editor, you should see the code Copilot generated for you

    !IMAGE[A picture of code Copilot generated](instructions281384/Screenshot 2025-01-21 at 1.53.43 PM.png){400}

1. You can press tab to accept the code or hover over the code with your mouse and click accept.

    !IMAGE[A picture of pressing tab to accept the code or hovering over the code with your mouse and clicking accept](instructions281384/Screenshot 2025-01-21 at 1.53.58 PM.png){400}

1. Once you accept the code, the color will change indicating the code has been accepted

    !IMAGE[A picture of the color changing indicating the code has been accepted](instructions281384/Screenshot 2025-01-21 at 1.54.12 PM.png){400}

    and you can run it with the run button in the query editor to see the results.

    !IMAGE[A picture of running the code with the run button in the query editor to see the results](instructions281384/Screenshot 2025-01-21 at 1.59.52 PM.png){500}
...
1. Now, delete the **5 after the word TOP in the first line of code**. 
    ...
    !IMAGE[A picture showing the 5 deleted in the query](media/image#.png){400}
...
1. You should get an error on the bottom of the page in the messages pane stating "Msg 102, Level 15, State 1, Line 4, Incorrect syntax near 'C'."
    ...
    !IMAGE[A picture of an error on the bottom of the page in the messages pane stating Msg 102, Level 15, State 1, Line 4, Incorrect syntax near C](media/image#.png){700}

1. Back on the query editor toolbar. click the **Fix query errors** button.

    !IMAGE[A picture of clicking the Fix query errors button](instructions281384/Screenshot 2025-01-21 at 2.05.02 PM.png){500}

1. You will see that it added back the 5 and notated the error with how it fixed it.

    !IMAGE[A picture of the query being fixed and an annotation on what it did](instructions281384/Screenshot 2025-01-21 at 2.07.02 PM.png){600}

1. Next, click the **Explain query button** on the query editor toolbar.

    !IMAGE[A picture of clicking the Explain query button on the query editor toolbar](instructions281384/Screenshot 2025-01-21 at 2.08.56 PM.png){500}

1. This option annotates your code for you adding comments outlining the purpose/function of each section.

    !IMAGE[A picture of the code being annotated automatically for you](instructions281384/Screenshot 2025-01-21 at 2.11.52 PM.png){600}
...
1. Then click the run button on the query sheet
    ...
    !IMAGE[A picture of clicking the run button on the query sheet](media/image#.png){900}
...
1. Click the run button on the query sheet. The result will be an amazing joke you can tell your friends and family!
    ...
    !IMAGE[A picture of running the T-SQL to create a llama bar joke](media/image#.png){1000}
...
1. Again, click the run button on the query sheet. The result will be a JSON vector array.
    ...
    !IMAGE[A picture of the JSON vector array as a result of the query](media/image#.png){1200}
...
1. Then click the run button on the query sheet
    ...
    !IMAGE[A picture of clicking the run button on the query sheet for adding 2 columns to the product table](instructions281384/Screenshot 2025-01-10 at 1.30.19 PM.png){800}
...
1. Run the next query in a blank query editor in Microsoft Fabric to see the results of the above update to the Products table:
    ...
    !IMAGE[A picture of the query result showing the chunk and embeddings columns and their data.](media/image#.png){1000}
...
1. The first query will pose the question "I am looking for a red bike and I dont want to spend a lot". The key words that should help with our similarity search are red, bike, and dont want to spend a lot. Run the following SQL in a blank query editor in Microsoft Fabric:
    ...
    !IMAGE[A picture of running Query 1 and getting results outlined in the Query 1 results table.](media/image#.png){800}
...
1. The next search will be looking for a safe lightweight helmet. Run the following SQL in a blank query editor in Microsoft Fabric:
    ...
    !IMAGE[A picture of running Query 2 and getting results outlined in the Query 2 results table](media/image#.png){800}
...
1. In the previous 2 examples, we were clear on what we were looking for; cheap red bike, light helmet. In this next example, we are going to have the search flex its AI muscles a bit by saying we want a bike seat that needs to be good on trails. This will require the search to look for adjacent values that have something in common with trails. Run the following SQL in a blank query editor in Microsoft Fabric to see the results.
    ...
    !IMAGE[A picture of running Query 3 and getting results outlined in the Query 3 results table](media/image#.png){800}
...
1. You can test this newly created procedure to see how it will interact with the GraphQL API by running the following SQL in a blank query editor in Microsoft Fabric:
    ...
    !IMAGE[A picture of running the find_products_api stored procedure](media/image#.png){800}

1. To create the GraphQL API, click on the **New API for GraphQL** button on the toolbar.

    !IMAGE[A picture of clicking on the New API for GraphQL button on the toolbar](media/image#.png){800}

1. In the **New API for GraphQL** dialog box, use the **Name Field** and name the API **find_products_api**.

    +++find_products_api+++

    !IMAGE[A picture of using the Name Field and naming the API find_products_api in the New API for GraphQL dialog box](media/image#.png){300}

1. After naming the API, click the **green Create button**.

    !IMAGE[A picture of clicking the green Create button in the New API for GraphQL dialog box](media/image#.png){300}
...
1.  The next dialog box presented is the **Choose data** dialog box where you will pick a table or stored procedure for the GraphQL API, 
    ...
    !IMAGE[A picture of the next dialog box being presented, the Choose data dialog box](media/image#.png)

    use the **Search box** in the **Explorer section** on the left 
    
    !IMAGE[A picture of using the Search box in the Explorer section on the left of the Choose data dialog box](media/image#.png){300}

    and **enter in find_products_api**.

    +++find_products_api+++

    !IMAGE[A picture of enter in find_products_api in the search box](media/image#.png){300}
...
1. Choose the **find_products_api stored procedure in the results**. You can ensure it is the find_products_api stored procedure by hovering over it with your mouse/pointer. It will also indicate the selected database item in the preview section. It should state **"Preview data: dbo.find_products_api"**.
    ...
    !IMAGE[A picture of choosing the find_products_api stored procedure in the results](instructions281384/Screenshot 2025-01-17 at 6.28.44 AM copy.png){700}
...
1. Once you have selected the **find_products_api stored procedure**, click the **green Load button** on the bottom right of the modal dialog box.
    ...
    !IMAGE[A picture of clicking the green Load button on the bottom right of the modal dialog box](media/image#.png){700}
...
1. You will now be on the **GraphQL Query editor page**. Here, we can run GraphQL queries similar to how we can run T-SQL queries on the query editor.
    ...
    !IMAGE[A picture of the GraphQL Query editor page](media/image#.png){700}
...
1. Replace the sample code on the left side of the GraphQL query editor with the following query:
    ...
    !IMAGE[A picture of replacing the example code with the query code provided](media/image#.png){700}

1. Now, click the **Run button** in the upper left of the GraphQL query editor. 

    !IMAGE[A picture of clicking the Run button in the upper left of the GraphQL query editor](media/image#.png){700}

    and you can scroll through the results from the GraphQL query in the lower section of the editor.

    !IMAGE[A picture of scrolling through the results from the GraphQL query in the lower section of the editor](media/image#.png){700}

1. Back on the toolbar, find and click the **Generate code button**.

    !IMAGE[A picture of clicking the Generate code button](media/image#.png){600}
...
1. This feature will generate the code for calling this API via python or node.js to help give you a jumpstart in the application creation process.
    ...
    !IMAGE[A picture of the generated code the editor provides](media/image#.png){800}
...
1. When done looking at the python and node.js code, click the **X** in the upper right corner to close the Generate code dialog box.
    ...
    !IMAGE[A picture of clicking the X in the upper right corner to close the Generate code dialog box](instructions281384/Screenshot 2025-01-15 at 8.40.12 AM2.png){500}
...
1. Before we can start creating new stored procedures, we need to go back to the SQL Database home page. Do this by using the navigator on the left side of the page and clicking on the SQL Database icon.
    ...
    !IMAGE[A picture of using the navigator on the left side of the page and clicking on the SQL Database icon](media/image#.png){200}
...
1. You can test this new  procedure to see how Azure OpenAI will answer a question with product data by running the following SQL in a blank query editor in Microsoft Fabric:
    ...
    !IMAGE[A picture of running the find_products_chat_api stored procedure](media/image#.png){800}
...
1. In the **New API for GraphQL** dialog box, use the **Name Field** and name the API **find_products_chat_api**.
    ...
    !IMAGE[A picture of using the Name Field and naming the API find_products_chat_api in the New API for GraphQL dialog box](media/image#.png){300}
...
1.  The next dialog box presented is again the **Choose data** dialog box where you will pick a table or stored procedure for the GraphQL API, 
    ...
    !IMAGE[ picture of enter in find_products_chat_api in the search box](media/image#.png){300}
...
1. Choose the stored procedure in the results. You can ensure it is the **find_products_chat_api stored procedure** by hovering over it with your mouse/pointer. It will also indicate the selected database item in the preview section. It should state **"Preview data: dbo.find_products_chat_api"**.
    ...
    !IMAGE[A picture of choosing the find_products_chat_api stored procedure in the results](media/image#.png){700}
...
1. Replace the sample code on the left side of the GraphQL query editor with the following query:
    ...
    !IMAGE[A picture of replacing the sample code on the left side of the GraphQL query editor with the supplied code](media/image#.png){600}

1. Now, **click the Run button** in the upper left of the GraphQL query editor.

    !IMAGE[A picture of clicking the Run button in the upper left of the GraphQL query editor](media/image#.png){600}    

1. And you can review the response in the **Results** section of the editor

    !IMAGE[A picture of reviewing the response in the Results section of the editor](media/image#.png){600}
...
1. Using the navigator on the left side of the Microsoft Fabric page, **click the tile for your database**. It will be named **sqlDatabase followed by a set of numbers**.
    ...
    !IMAGE[A picture of using the navigator on the left side of the Microsoft Fabric page to click the tile for the database](media/image#.png){300}

1. Now on the database details page, find the **SQL database dropdown** in the upper right of the page.

    !IMAGE[A picture of finding the SQL database dropdown in the upper right of the page](media/image#.png){600}

1. Click the SQL database dropdown and select **SQL analytics endpoint**.

    !IMAGE[A picture of clicking the SQL database dropdown and selecting SQL analytics endpoint](media/image#.png){500}
    ...
    !IMAGE[A picture of clicking on your workspace tile, and selecting the SQL analytics endpoint on the bottom of the page](media/image#.png){600}

1. On the SQL database analytics endpoint details page, **click the Reporting tab** from the ribbon.

    !IMAGE[A picture of clicking the Reporting tab from the ribbon on the SQL database analytics endpoint details page](media/image#.png){600}

1. Now, click the **New Report** button.

    !IMAGE[A picture of clicking the New Report button](media/image#.png){600}
...
1. When the **New report with all available data modal window** opens, click the **green Continue button** to create a new report with all available data.
    ...
    !IMAGE[A picture of clicking the green Continue button to create a new report in the New report with all available data modal window](media/image#.png){600}

1. The **Upgrade to a paid Power BI license modal window** is next. Simply **click the white Try free button**.

    !IMAGE[A picture of clicking the white Try free button in the Upgrade to a paid Power BI license modal window](media/image#.png){700}
    
1. And confirm the free license on the following model by **clicking the green Got it button**.

    !IMAGE[A picture of clicking the green Got it button on the following model](media/image#.png){600}

1. Once the new report opens in Power BI, click the **Try it** button in the **green callout box for Copilot**.

    !IMAGE[A picture of clicking the Try it button in the **green callout box for Copilot](media/image#.png){600}

1. On the **right side** of the page, the **Copilot blade** opens.

    !IMAGE[A picture of copilot blade on the right side of the page](media/image#.png){400}

    Click the **green Get started button**.

    !IMAGE[A picture of clicking the green Get started button in the copilot blade](media/image#.png){400}

1. Copilot presents some questions to get you started. Click **Suggest content for a new report page**.

    !IMAGE[A picture of clicking Suggest content for a new report page in the copilot blade](media/image#.png){400}
...
1. Suggested outlines for your report are returned by Copilot based on the data it has access to in the analytics endpoint. 
    ...
    !IMAGE[A picture of suggested outlines for a report returned by Copilot based on the data it has access to in the analytics endpoint](media/image#.png){300}

1. You can review the selections by **expanding their cards** to see a quick description about the report.

    !IMAGE[A picture of expanding report suggestion cards to see a quick description about the report](media/image#.png){300}

1. Click the **Create button** for the **Sales Order Details** report **or any other report** you would like to create. Go wild!

    !IMAGE[A picture of clicking the Create button for the Sales Order Details report](media/image#.png){300}

1. Copilot will then begin **creating your report**.

    !IMAGE[A picture of copilot creating a Power BI report](media/image#.png){300}

1. You now have a Power BI report of your SQL database data!

    !IMAGE[A picture of the copilot generated power bi report](media/image#.png){700}

1. **Minimize the blades** 

    !IMAGE[A picture of minimizing the blades to make the report larger](media/image#.png){600}

    to expand the size of the report.

    !IMAGE[A picture of a Power BI report with the blades minimized](media/image#.png){600}
