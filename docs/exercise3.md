...
1. Using an empty query sheet in Microsoft Fabric, copy and paste the following code:
    ...
    if not exists(select * from sys.database_scoped_credentials where [name] = '<inject key="openaiendpoint"></inject>')
    begin
        create database scoped credential [<inject key="openaiendpoint"></inject>]
        with identity = 'HTTPEndpointHeaders', secret = '{"api-key":"<inject key="openAIKey"></inject>"}';
    end
    go
    ```
...
1. The result will be a single line joke generated by Openai model.
...
1. Let's start with Content Safety. Copy and paste the following code into a blank query editor in Microsoft Fabric:
    ...
    if not exists(select * from sys.database_scoped_credentials where [name] = '<inject key="contentsafetyendpoint"></inject>')
    begin
        create database scoped credential [<inject key="contentsafetyendpoint"></inject>]
        with identity = 'HTTPEndpointHeaders', secret = '{"Ocp-Apim-Subscription-Key":"<inject key="ContentSafetyKey"></inject>","Ocp-Apim-Subscription-Region":"<inject key="region"></inject>"}';
    end
    go
    ```
...
1. Using the following code, we can use the Text Analysis feature of Azure AI Content Safety. Copy and paste the following code into a blank query editor in Microsoft Fabric:
    ...
    ```SQL
    declare @url nvarchar(4000) = N'<inject key="contentsafetyendpoint"></inject>contentsafety/text:analyze?api-version=2024-09-15-preview';
    ...
    exec @ret = sp_invoke_external_rest_endpoint
         ...
         @method = 'POST',
         @payload = @payload,
         @credential = [<inject key="contentsafetyendpoint"></inject>],
         @timeout = 230,
         @response = @response output;
...
1. Now, let's try the Translation service. Copy and paste the following code into a blank query editor in Microsoft Fabric:
    ...
    begin
        ...
        with identity = 'HTTPEndpointHeaders', secret = '{"Ocp-Apim-Subscription-Key":"<inject key="TranslatorKey"></inject>","Ocp-Apim-Subscription-Region":"<inject key="region"></inject>"}';
    end
    go
    ```
...
1. Using an empty query sheet in Microsoft Fabric, copy and paste the following code. This code calls an Azure OpenAI embeddings endpoint. The result will be a JSON array of vectors.
    ...
    ```SQL
    declare @url nvarchar(4000) = N'<inject key="openaiendpoint"></inject>openai/deployments/text-embedding-ada-002/embeddings?api-version=2024-06-01';
...
1. In a new query sheet or an existing blank sheet, copy and paste the following T-SQL:
...
1. Next, we are going to use the External REST Endpoint Invocation procedure (sp_invoke_external_rest_endpoint) to create a stored procedure that will create embeddings for text we supply as input. Copy and paste the following code into a blank query editor in Microsoft Fabric:
    ...
    AS
    BEGIN
    declare @url varchar(max) = '<inject key="openaiendpoint"></inject>openai/deployments/text-embedding-ada-002/embeddings?api-version=2024-06-01';
...
1. The first query will pose the question "I am looking for a red bike and I dont want to spend a lot". The keywords that should help with our similarity search are red, bike, and dont want to spend a lot. Run the following SQL in a blank query editor in Microsoft Fabric:
